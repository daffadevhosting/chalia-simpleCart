...
function WhatsApp() {
 ...
 waItems.forEach((item,i)=>{ 
    ...
    cartItem += '*Harga /bungkus :* '+angkaToRp(item.price)+' %0A';
    cartItem += '*Berat Isi :* '+item.ukuran+' %0A';
    ...
    counter++;
               ...
               'Hallo kak, saya %2A' + nama + '%2A, mau order ini.%0A' +
               'Mohon diproses %26 segera dikirim, yaaa. %2ATerima Kasih%2A%0A%0A' +
               '*Order ID :* ' +
                + d + '%0A%0A' +
               '-----------------------------------------%0A' +
               ...
               '------------------------------------------%0A' +
               ' %0A%2ACatatan%2A: Tolong kirim link pembayaran nya, ya kak...' );
				var w = 960,
					h = 540,
